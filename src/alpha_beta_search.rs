...
pub fn get_best(game:&mut game::GameInfo,depth:i8, net: &model::Net) -> move_gen::Move{
    ...
    for mut mov in moves{
        ...
        let score = alpha_beta_min_net(tch::Tensor::of_slice(&[f64::MIN]),tch::Tensor::of_slice(&[f64::MAX]),depth-1,game,net).f_double_value(&[0]).unwrap();
...
pub fn alpha_beta_max_net(alpha:tch::Tensor,beta:tch::Tensor,depth:i8,game:&mut game::GameInfo, net: &model::Net) -> tch::Tensor {
    if depth == 0{
...
pub fn alpha_beta_min_net(alpha:tch::Tensor,beta:tch::Tensor,depth:i8,game:&mut game::GameInfo, net: &model::Net) -> tch::Tensor {
