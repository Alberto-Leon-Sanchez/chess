...
use std::{fs::{File, OpenOptions, self}, io::{BufReader, Write, BufRead}};

use regex;
use tch::nn::{self, Module};

use crate::{move_gen::{self, move_gen}, game, fen_reader, notation, model, eval, make_move, unmake};

const UNINITIALIZED:f64 = 10.0;

pub fn test_model() -> (){

    let paths:Vec<String> = fs::read_dir("./model_weights/").unwrap().into_iter().map(|x| x.unwrap().path().to_str().unwrap().to_string()).collect();
    ...
    for path in paths{
        vs.load_from_stream(&mut BufReader::new(File::open(&path).unwrap())).unwrap();
        total_score = 0;
        ...
        suite_results.write_all(format!("{}: {}\n",path,total_score).as_bytes()).unwrap();
    }
...
fn get_suites() -> (Vec<game::GameInfo>, Vec<Vec<(move_gen::Move,i64)>>){
    ...
    let paths:Vec<String> = fs::read_dir("./suites/").unwrap().into_iter().map(|x| x.unwrap().path().to_str().unwrap().to_string()).collect();
    ...
    for path in paths{
        ...
        for line in buffer.lines(){
            let line = line.unwrap().split("bm").into_iter().map(|x| x.to_string()).collect::<Vec<String>>();
            ...
            for capture in regex.captures_iter(&line[1]){
                ...
                if capture.get(0).unwrap().as_str() != ""{
                    ...
                    let tmp = capture.get(0).unwrap().as_str().split("=").into_iter().map(|x| x.to_string()).collect::<Vec<String>>();
