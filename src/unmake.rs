use crate::api::get_moves;
use crate::game;
use crate::move_gen;
use crate::piece;

//TODO: unmake castling queen side weird stuff going on
...
pub fn unmake_move(mut game:game::GameInfo,movement:move_gen::Move) -> game::GameInfo{
    ...
    if let Some(piece_type) = movement.promotion{
        match game.turn{
            game::Color::Black => {
                ...
                game.black_pieces.add_piece(&piece::PieceType::Pawn, &movement.origin);
            },
            game::Color::White => {
                ...
                game.white_pieces.add_piece(&piece::PieceType::Pawn, &movement.origin);
            },
        }
    }else{
    ...
    match game.en_passant.last() {
        Some(en_passant) => {
            match en_passant{
                Some(pos) if matches!(origin_piece,piece::PieceType::Pawn) && movement.destiny == *pos => {
    ...
    match game.board[movement.origin as usize]{
        piece::Piece::White(p) => game.white_pieces.make_move(&p,movement.origin, movement.destiny),
        piece::Piece::Black(p) => game.black_pieces.make_move(&p,movement.origin, movement.destiny),
        _ => (),
    }
